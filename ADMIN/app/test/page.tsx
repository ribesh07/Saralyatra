...
const Dashboard = () => {
  ...
  // Bus CRUD handlers
  const handleBusFormChange = (e: { target: { name: any; value: any; }; }) => {
  ...
  const editBus = (bus: React.SetStateAction<{ id: string; name: string; capacity: string; }>) => {
    setBusForm(bus);
  ...
  const editUser = (user: React.SetStateAction<{ id: string; name: string; email: string; }>) => {
    setUserForm(user);
            ...
            {buses.map((bus) => (
              <tr key={bus.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <button
                    onClick={() => editBus({ ...bus, capacity: String(bus.capacity) })}
