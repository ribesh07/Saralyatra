...
import 'package:saralyatra/Booking/seatbooking_form.dart';
import 'package:saralyatra/pages/setups/seats_layout.dart';
...
class SeatS extends StatefulWidget {
  ...
  }) : super(key: key ?? ValueKey(uniqueIDs));
...
//..
  ...
  Future<void> _bookSeats() async {
    if (selectedSeats.isNotEmpty) {
      ...
      print(selectedSeats);
      String response = await Navigator.pushReplacement(
        context,
        MaterialPageRoute(
      ...
      Future<void> _resetSeatBooking() async {
        if (selectedSeats.isEmpty) {
          print('No seats selected for reset.');
          return;
        }

        // Build reset map only for selected seat IDs
        final Map<String, dynamic> resetData = {};

        for (var seat in selectedSeats) {
          resetData['book$seat'] = false;
        }
        ...
        try {
          ...
          await docRef.update(resetData); // only update selected seats
          print('✅ Reset only selected seats: $resetData');
        } catch (e) {
          print('❌ Error resetting selected seats: $e');
        }
      }

      if (response == 'reset') {
        await _resetSeatBooking();
        await _blockBooked(); // 👈 Call the reset function here
        setState(() {
          selectedSeats.clear(); // Optional: clear UI state
        });
      }
    }
  }
