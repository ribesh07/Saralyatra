...
class _BusControlPanelState extends State<BusControlPanel> {
  ...
  Future<void> fetchAllDriverRoutes() async {
    try {
      ...
      final List<Map<String, dynamic>> fetchedRoutes = snapshot.docs.map((doc) {
        ...
        // Ensure every field is non-null using fallback/defaults
        return {
          'busNo': data['busNo']?.toString() ?? 'Unknown Bus No',
          'route': data['title']?.toString() ?? 'No Route Title',
  ...
  Widget _buildBusList(List<Map<String, dynamic>> buses,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final bus = buses[index];
        final busNo = bus['busNo'] ?? 'Unknown';
        final route = bus['route'] ?? 'Unknown Route';

        return GestureDetector(
          onTap: active ? () => _navigateToMap(route) : null,
          child: Card(
            elevation: active ? 4 : 1,
            color: active ? null : Colors.grey[200],
            child: ListTile(
